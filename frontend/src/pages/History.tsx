...
export function History() {
  ...
  const handleExport = async () => {
    try {
      const items = await historyApi.export()
      downloadJson('history-export.json', { schemaVersion: 1, type: 'history', data: items })
      setToast({ message: 'History exported', kind: 'success' })
    } catch {
  ...
  const handleImport = async (event: any) => {
    ...
    try {
      ...
      const summary = await historyApi.import(payload)
      qc.invalidateQueries({ queryKey: ['history'] })
      setToast({ message: `${summary.added} added · ${summary.updated} updated`, kind: 'success' })
    } catch {
    ...
    <div className="p-6">
      <div className="flex items-center justify-between mb-6 gap-3">
        ...
        <div className="flex items-center gap-2">
          ...
          <button onClick={() => document.getElementById('history-import-input')?.click()} className="btn-secondary text-sm flex items-center gap-1.5">
      ...
      <input id="history-import-input" type="file" accept="application/json" className="hidden" onChange={handleImport} />
      ...
      {!data?.length ? (
        <EmptyState icon="🕐" title="No watch history" subtitle="Videos you watch will appear here." />
      ) : (
